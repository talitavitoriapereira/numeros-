...
db.serialize(() => {
    ...
    `,
        (err) => {
            if (err)
                console.error(
                    "Erro ao criar tabela encaminhamento:",
                    err.message,
                );
    ...
    `,
        (err) => {
            if (err)
                console.error("Erro ao criar tabela frequencia:", err.message);
...
app.post("/aluno", (req, res) => {
    ...
    if (!nome || !cpf || !rg || !cpf_responsavel || !curso || !turno) {
        return res.status(400).json({
            error: "Nome, CPF, RG, CPF responsável, curso e turno são obrigatórios.",
        });
    }
    ...
    db.run(
        ...
        function (err) {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ error: "Erro ao cadastrar aluno." });
            }
            res.status(201).json({
                id: this.lastID,
    ...
    if (cgm) {
        ...
        db.all(query, [`%${cgm}%`], (err, rows) => {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ error: "Erro ao buscar alunos." });
            }
            res.json(rows);
        });
    } else {
...
app.post("/funcionario", (req, res) => {
    ...
    if (!nome || !cpf || !rg) {
        return res
            .status(400)
            .json({ error: "Nome, CPF e RG são obrigatórios." });
    }
    ...
    db.run(
        ...
        function (err) {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ error: "Erro ao cadastrar funcionário." });
            }
            res.status(201).json({
                id: this.lastID,
    ...
    if (cpf) {
        ...
        db.all(query, [`%${cpf}%`], (err, rows) => {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ error: "Erro ao buscar funcionários." });
            }
            res.json(rows);
        });
    } else {
    ...
    db.run(
        ...
        function (err) {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ error: "Erro ao atualizar funcionário." });
            }
            if (this.changes === 0) {
                return res
                    .status(404)
...
app.post("/fo", (req, res) => {
    ...
    if (!data || !turma || !tipo_fato || !monitor) {
        return res.status(400).json({
            error: "Data, turma, tipo_fato e monitor são obrigatórios.",
        });
    }
...
app.post("/ata", (req, res) => {
    ...
    if (!prof || !monitor || !aluno || !assunto || !dia || !cgm) {
        return res.status(400).json({
            error: "Prof, monitor, aluno, assunto, dia e CGM são obrigatórios.",
        });
    }
    ...
    db.run(
        query,
        [aluno, dia, assunto, monitor, conteudo, encaminhamento, cgm, prof],
        function (err) {
            if (err) {
                console.error("Erro no INSERT ata:", err.message);
                return res
                    .status(500)
                    .json({ error: "Erro ao cadastrar ata." });
            }
            res.status(201).json({
                id: this.lastID,
...
app.put("/ata/:id", (req, res) => {
    ...
    db.run(
        query,
        [aluno, dia, assunto, monitor, conteudo, encaminhamento, cgm, prof, id],
        function (err) {
            if (err) {
                console.error("Erro no UPDATE ata:", err.message);
                return res
                    .status(500)
                    .json({ error: "Erro ao atualizar ata." });
            }
            if (this.changes === 0) {
...
app.post("/encaminhamento", (req, res) => {
    ...
    if (!destinatario || !data) {
        return res
            .status(400)
            .json({ error: "Destinatário e data são obrigatórios." });
    }
    ...
    db.run(query, [data, destino, destinatario, obs, aluno], function (err) {
        if (err) {
            ...
            return res
                .status(500)
                .json({ error: "Erro ao cadastrar encaminhamento." });
        }
        res.status(201).json({
            id: this.lastID,
    ...
    if (data) {
        ...
        db.all(query, [`%${data}%`], (err, rows) => {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ error: "Erro ao buscar encaminhamentos." });
            }
            res.json(rows);
        });
    } else {
...
app.put("/encaminhamento/:id", (req, res) => {
    ...
    db.run(
        query,
        [data, destino, destinatario, obs, aluno, id],
        function (err) {
            if (err) {
                console.error("Erro no UPDATE encaminhamento:", err.message);
                return res
                    .status(500)
                    .json({ error: "Erro ao atualizar encaminhamento." });
            }
            if (this.changes === 0) {
                return res
                    .status(404)
                    .json({ error: "Encaminhamento não encontrado." });
            }
...
app.post("/frequencia", (req, res) => {
    const { aluno, turma, ausencias, data_aula, cgm, materia, justificativa } =
        req.body;

    if (!aluno || !turma || ausencias === undefined || !data_aula || !cgm) {
        return res.status(400).json({
            error: "Aluno, turma, ausências, data da aula e CGM são obrigatórios.",
        });
    }
    ...
    db.run(
        query,
        [aluno, turma, ausencias, data_aula, cgm, materia, justificativa],
        function (err) {
            if (err) {
                console.error("Erro no INSERT frequencia:", err.message);
                return res
                    .status(500)
                    .json({ error: "Erro ao cadastrar frequência." });
            }
            res.status(201).json({
                id: this.lastID,
    ...
    if (aluno) {
        ...
        db.all(query, [`%${aluno}%`], (err, rows) => {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ error: "Erro ao buscar frequências." });
            }
            res.json(rows);
        });
    } else if (turma) {
...
app.put("/frequencia/:id", (req, res) => {
    ...
    db.run(
        query,
        [aluno, turma, ausencias, data_aula, cgm, materia, justificativa, id],
        function (err) {
            if (err) {
                console.error("Erro no UPDATE frequencia:", err.message);
                return res
                    .status(500)
                    .json({ error: "Erro ao atualizar frequência." });
            }
            if (this.changes === 0) {
                return res
                    .status(404)
                    .json({ error: "Frequência não encontrada." });
            }
...
app.post("/turma", (req, res) => {
    ...
    if (
        ...
        return res.status(400).json({
            error: "nome, codigo, curso, ano , ano letivo e coordenador são obrigatórios.",
        });
    }

    const query = `INSERT INTO turma (nome_turma,codigo,turno,curso,ano,ano_letivo,capacidade,sala,coordenador) VALUES (?,?,?,?,?,?,?,?,?)`;
    db.run(
        query,
        [
            nome_turma,
        ...
        function (err) {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ error: "Erro ao cadastrar turma." });
            }
            res.status(201).json({
                id: this.lastID,
...
app.get("/turma", (req, res) => {
    ...
    if (codigo) {
        const query = `SELECT * FROM turma WHERE codigo LIKE ?`;
    ...
    db.run(
        ...
        function (err) {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ error: "Erro ao atualizar turma." });
            }
            if (this.changes === 0) {
